...
import { booksRequest, prepareJournalDraft, startGoogleDriveConnection } from '../../hostinger/supabase';
import {
  BookOpenCheck,
  Plus,
  ArrowLeft,
...
export default function Workspace({ user }: { user: string }) {
  ...
  async function connectDrive() {
    if (!active) return;
    setBusy(true);
    try { await startGoogleDriveConnection(active); }
    catch (e: any) { setError(e.message || 'Unable to start the secure Google Drive connection.'); setBusy(false); }
  }
    ...
    <main className="workspace" lang={hi ? 'hi' : 'en'}>
      ...
      <div className="workspace-main">
        ...
        <div className="workspace-title">
          ...
          {active && <button className="quiet" onClick={connectDrive} disabled={busy}>Connect Google Drive</button>}
        </div>
        {businesses.length > 0 && (
