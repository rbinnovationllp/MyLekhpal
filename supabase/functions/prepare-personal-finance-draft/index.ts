...
function personalSkillRepository(): SkillRepository {
  const base = new URL('./skill-source/', import.meta.url);
  const references = ['bank-statement-analysis.md', 'household-budgeting.md', 'liabilities-and-goals.md', 'retirement-planning.md', 'tax-service-intake.md', 'professional-referrals.md', 'regulated-advice-boundaries.md', 'validation-scenarios.md', 'output-schemas.md'];
  return {
    name: 'mylekhpal-personal-finance-tax-support',
    description: 'Analyse household finances, personal tax-support documents, budgets, liabilities, goals and retirement scenarios; never use for business journal preparation.',
    skillPath: '/skills/mylekhpal-personal-finance-tax-support/SKILL.md',
    loadSkill: () => Deno.readTextFile(new URL('SKILL.md', base)),
    references: Object.fromEntries(references.map((name) => [`/skills/mylekhpal-personal-finance-tax-support/references/${name}`, () => Deno.readTextFile(new URL(`references/${name}`, base))])),
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await admin.from('private.personal_ai_processing_logs').insert({ household_id: input.householdId, request_id: provisionalRequestId, skill_name: 'mylekhpal-personal-finance-tax-support', skill_version: '1.0.1', provider: 'anthropic', status: 'started', input_hash: inputHash });
    const agent = await runProgressiveSkillAgent({
      repository: personalSkillRepository(), model: Deno.env.get('MYLEKHAPAL_ANTHROPIC_MODEL') || 'claude-sonnet-4-6', apiKey: Deno.env.get('MYLEKHAPAL_ANTHROPIC_API_KEY')!, maxTokens: 4500,
