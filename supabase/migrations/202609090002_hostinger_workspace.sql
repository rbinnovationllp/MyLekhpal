-- Hostinger browser API: atomic writes, authenticated tenant checks, no service key in browser.
begin;
alter table public.businesses add column if not exists workspace_settings jsonb not null default '{}'::jsonb;
-- Profiles were omitted from the original migration's RLS list.
  ...
  if action = 'read' then
    return jsonb_build_object(
      'business', jsonb_build_object('id',b.id,'name',b.legal_name,'trade_name',b.display_name,'client_id',b.workspace_settings->>'clientId',
        'year',b.workspace_settings->'year','status','Onboarding draft','active_role',role_name),
...
$$;
revoke all on function public.mylekhpal_books(jsonb) from public, anon;
grant execute on function public.mylekhpal_books(jsonb) to authenticated;
