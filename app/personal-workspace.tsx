...
export default function PersonalWorkspace() {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => {
    if (!householdId) { setGoogleDrive({ connected: false }); return; }
    let valid = true;
    googleDriveRequest('google-drive-status', 'personal', householdId).then((status) => { if (valid) setGoogleDrive(status); }).catch(() => { if (valid) setGoogleDrive({ connected: false }); });
    return () => { valid = false; };
  }, [householdId]);
  async function connectGoogleDrive() {
    ...
    try {
      const result = await googleDriveRequest('google-drive-connect', 'personal', householdId);
      if (result.connected) { setGoogleDrive(result); setNotice('Google Drive is already connected for this personal workspace.'); return; }
  ...
  async function create(event: React.FormEvent) {
    ...
    try {
      ...
      setNotice('Your workspace was created. Continue to secure AutoPay authorisation to begin your 3-day trial.');
